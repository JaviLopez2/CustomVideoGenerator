                ...
                for (_node_id, node_data) in workflow_prompt {
                    if let Some(class_type_ref) = node_data["class_type"].as_str() {
                        ...
                        match class_type.as_str() {
                            // MPT_DYNAMIC_REFERENCE_PATCH: FLUX.2 RandomNoise seed support
                            "RandomNoise" => {
                                if let Some(inputs_data_noise) = node_data["inputs"].as_object_mut()
                                {
                                    if let Some(seed_data) =
                                        openai_request.get("seed").and_then(|v| v.as_i64())
                                    {
                                        inputs_data_noise.insert(
                                            "noise_seed".to_string(),
                            ...
                            "KSampler" | "easy seed" => {
                                    ...
                                    node_data["inputs"].as_object_mut()
                                {
                                    ...
                                    // MPT_QWEN21_PERFORMANCE_PATCH: profile-aware step count.
                                    if let Some(steps_data) =
                                        openai_request.get("steps").and_then(|v| v.as_i64())
                                    {
                                        let safe_steps = steps_data.clamp(1, 200);
                                        inputs_data_sampler.insert(
                                            "steps".to_string(),
                                            serde_json::json!(safe_steps),
                                        );
                                        debug!(
                                            "┃ [{}] ✏️ Requested sampler steps: {}",
                                            req_id, safe_steps
                                        );
                                    }
                                }
                            ...
                            // MPT_DYNAMIC_REFERENCE_PATCH: keep Flux2Scheduler in sync with requested size
                            "Flux2Scheduler" => {
                                if let Some(inputs_data_size) = node_data["inputs"].as_object_mut()
                                {
                                    if let Some(size_data) =
                            ...
                            "LoadImage" => {
                                ...
                                let request_field = if node_title.contains("Style Reference") {
                                    "style_reference_image".to_string()
                                } else if let Some(index_text) =
                                    node_title.strip_prefix("Subject Reference ")
                                {
                                    match index_text.trim().parse::<usize>() {
                                        Ok(index) if index > 1 && index <= 10 => {
                                            format!("reference_image_{}", index)
                                        }
                                        _ => "reference_image".to_string(),
                                    }
                                } else {
                                    // Backwards compatible with the existing FLUX workflow title.
                                    "reference_image".to_string()
                                };

                                if let Some(inputs_data_image) = node_data["inputs"].as_object_mut() {
                                    let specific_reference = openai_request
                                        .get(&request_field)
                                        .and_then(|v| v.as_str())
                                        ...
                                        specific_reference.or(fallback_reference)
                                    {
                                        inputs_data_image.insert(
                                            "image".to_string(),
                                            Value::String(reference_image.to_string()),
                                        );
                                        debug!(
                                            "┃ [{}] Dynamic {}: {}",
                                            req_id, request_field, reference_image
                                        );
                                    }
                                }
                            }
                            ...
                            "TextEncodeQwenImage21" => {
                                if let Some(inputs_data) = node_data["inputs"].as_object_mut() {
                                    ...
                                    {
                                        let qwen_prompt = if prompt_input.contains("<image1>") {
                                            prompt_input.to_string()
                                        } else {
                                            format!(
                                                "Use <image1>, <image2> and <image3> as identity references for the same factual subject. Preserve the shared real-world identity and subject-defining physical features, but follow the requested scene, camera, environment and lighting instead of copying the reference composition. Do not inherit watermark text, stock-site text, labels or license-plate text from the references. Requested scene: {}",
                                                prompt_input
                                            )
                                        };
                                        inputs_data.insert(
                                            "prompt".to_string(),
                                            Value::String(qwen_prompt),
                                        );
                                    } else {
                                        return Err(ProxyError::Json(format!(
                                ...
                                if let Some(meta_data) = node_data["_meta"].as_object() {
                                    if let Some(title) = meta_data["title"].as_str() {
                                        ...
                                        if title == "Positive Prompt"
                                            || title.contains("Positive Prompt")
                                            || title == "CLIP文本编码"
                                            || title == "🔤 CR Text"
                                            || title == "提示词行"
                                        {
